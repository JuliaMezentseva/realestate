...
(async () => {
  try {
    ...
    setVal(w, stepTitleInput, "Получить доступ к базе объектов");
    await tick(50);
    ...
    const firstCatalogRow = [...dlg.querySelectorAll(".sk-clickable")].find(el => el.textContent.includes("Этапы сделки на вторичном рынке: от звонка до регистрации"));
    ...
    await tick(80);
    dlg = topDialog(w);
    console.log("Material appears in step form list:", dlg.textContent.includes("Этапы сделки на вторичном рынке: от звонка до регистрации") ? "PASS" : "FAIL");

    const saveStepBtn = [...dlg.querySelectorAll("button")].filter(b => b.textContent.trim() === "Добавить" && !b.disabled).pop();
    click(w, saveStepBtn);
    await tick(80);
    body = w.document.body.textContent;
    console.log("Step visible in goal drawer after save:", body.includes("Получить доступ к базе объектов") ? "PASS" : "FAIL");
    ...
    await tick(80);
    body = w.document.body.textContent;
    console.log("Step title visible directly in goals list (not just inside drawer):", body.includes("Получить доступ к базе объектов") ? "PASS" : "FAIL");
    ...
    const subgoalRow = [...w.document.querySelectorAll("div")].find(el =>
      el.textContent.includes("Получить доступ к базе объектов") && el.textContent.includes("Полезные материалы: 1") && el.querySelector("span"));
