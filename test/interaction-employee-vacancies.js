...
(async () => {
  try {
    ...
    console.log("Открытые вакансии видны по умолчанию (v1 'Специалист по недвижимости'):", body.includes("Специалист по недвижимости, вторичный рынок") ? "PASS" : "FAIL");
    ...
    // Отметка "Вы откликнулись" — есть отклик на v3 (Консультант по продаже новостроек) по данным myResponses
    console.log("Отметка 'Вы откликнулись' на вакансии с уже отправленным откликом:", /Консультант по продаже новостроек[\s\S]{0,400}Вы откликнулись/.test(body) || /Вы откликнулись[\s\S]{0,400}Консультант по продаже новостроек/.test(body) ? "PASS" : "FAIL");
    ...
    console.log("После включения чекбокса — открытые вакансии по-прежнему видны:", body.includes("Специалист по недвижимости, вторичный рынок") ? "PASS" : "FAIL");
    ...
    await tick(80);
    body = w.document.body.textContent;
    console.log("После фильтра по направлению — видна вакансия по новостройкам:", body.includes("Консультант по продаже новостроек") ? "PASS" : "FAIL");
    ...
    await tick(50);
    const spbOption = Array.from(w.document.querySelectorAll("li, [role='option'], div")).find(el => el.textContent.trim() === "Красногорск");
    spbOption.dispatchEvent(new w.MouseEvent("click", { bubbles: true }));
    await tick(80);
    body = w.document.body.textContent;
    console.log("После фильтра по региону 'Красногорск' — видна вакансия оттуда:", body.includes("Специалист по загородной недвижимости") ? "PASS" : "FAIL");
    console.log("После фильтра по региону 'Красногорск' — не видна московская вакансия:", !body.includes("Начальник отдела") ? "PASS" : "FAIL");
    ...
    console.log("Поиск по названию не показывает нерелевантные вакансии:", !body.includes("Консультант по продаже новостроек") ? "PASS" : "FAIL");
