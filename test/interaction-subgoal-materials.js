...
(async () => {
  try {
    ...
    const goalTitle = [...w.document.querySelectorAll("div")].find(el => el.textContent.trim() === "Провести первые показы и встречи с собственниками");
    ...
    console.log("Дровер шага открыт (заголовок цели виден):", body.includes("Провести первые показы") ? "PASS" : "FAIL");
    console.log("Блок 'Полезные материалы' показан со счётчиком 2/3:", body.includes("Полезные материалы 2/3") ? "PASS" : "FAIL");
    console.log("Материал-курс по скрипту звонка виден:", body.includes("Холодный звонок собственнику: структура разговора и скрипт") ? "PASS" : "FAIL");
