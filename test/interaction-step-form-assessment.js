...
(async () => {
  try {
    ...
    click(assessRow, w1);
    await tick(120);
    check("[1c] после клика появился select с готовыми листами", w1.document.body.textContent.includes("Холодный звонок собственнику"));
    ...
    const stepRows = [...w2.document.querySelectorAll(".sk-row.sk-clickable, .sk-clickable")]
      .filter(el => el.textContent.includes("Провести первый показ квартиры под наблюдением наставника") && el.className.includes("sk-row"));
    ...
    check("[2] файл-вложение сотрудника в комментарии виден", w2.document.body.textContent.includes("Отчёт по показу.pdf"));
    ...
    click(confirmBtn, w2);
    await tick(150);
    check("[2] после подтверждения дровер закрылся (решение применено)", !w2.document.body.textContent.includes("Оценочный лист: показ объекта"));
